...
object AdminClient {
  def create[F[_] : ConcurrentEffect](cfg: GrpcPubsubConfig): Resource[F, AdminClient[F]] = {
    ...
    val res: Resource[F, ManagedChannel] = ManagedChannelBuilder
      .forTarget(cfg.host)
      .resource[F]

    // I have no idea why the compiler won't let me go `res.map(...)`
    Resource.catsEffectMonadForResource[F].map(res)(channel => new AdminClient[F](channel, cfg.callOps))
  }
}
